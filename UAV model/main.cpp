...
int main(int argc, char** argv)
{
    ...
    */
	...
	cost2 = eucl_distance(finalPath[0].self_x,finalPath[0].self_y,finalPath[0].self_z,finalPath[record-1].self_x,finalPath[record-1].self_y,finalPath[record-1].self_z);
    cout<<"\nCost2 : "<<cost2;
...
bool noCollision(int xIn, int yIn,float hIn, int xFin, int yFin, float hFin)
{
    ...
    float m = (yFin - yIn)/(xFin - xIn);
    float c = yFin - (m*xFin);
    float baseSmall = 0;

    int x, y = 0;
    float dist = eucl_distane_2d(xIn,yIn,xFin,yFin);
    float height = abs(hFin - hIn);
    float ratio_bh = height/dist;
    float h;

    for(x = xIn ; x<= xFin; x++)
    {
        y = (m*x) + c;
        baseSmall = eucl_distane_2d(xIn,yIn,x,y);
        h = (baseSmall * ratio_bh) + hIn;

        if(h < _terrain->getHeight(x,y)+2)
            return false;
...
double costEstimate()
{
    ...
    {
        cost = cost + eucl_distance(finalPath[i].self_x, finalPath[i].self_y, finalPath[i].self_z, finalPath[i+1].self_x, finalPath[i+1].self_y, finalPath[i+1].self_z);
    }
    return cost;
}
